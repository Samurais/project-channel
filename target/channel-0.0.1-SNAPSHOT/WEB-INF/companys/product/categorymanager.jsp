...
<div id="thispage" title="商品类别管理<small>category manager</small>">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form id="defaultForm" method="post" class="form-horizontal"
						action="category_save.action">
						<div class="form-group">
							<label class="col-lg-3 control-label">类别编号</label>
							<div class="col-lg-5">
								<input type="text" class="form-control" name="id" />
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-3 control-label">类别数量</label>
							<div class="col-lg-5">
						...
						<div class="form-group">
							<div class="col-lg-4 col-lg-offset-1">
								<button id="btn_sub" type="button"
									class="btn btn-block btn-primary">添加类别</button>
							</div>
...
<script>
	...
	$(function() {
		$("#btn_sub").click(function() {
			var div = $("#defaultForm");
			$.post('category_save.action', {
				id : div.find("input[name=id]:first").val(),
				account : div.find("input[name=account]:first").val(),
	...
	//-- ==================================数据初始化部分============================================ -->
	//-- 后期在登陆的时候初始化
	$("#cpId").attr('value', getCookie("companyid"));
	$(function() {
		//1.初始化table
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'category_pageQuery.action', //请求后台的URL（*）
				...
				onEditableSave : function(field, row, oldValue, $el) {
					...
					$.ajax({
						type : "post",
						url : "category_editUpdate.action",
						data : {
							'id' : row.id,
							'type' : row.type,
							'account' : row.account,
		...
		oInit.Init = function() {
			...
			$("#btn_delete").click(function() {
				...
				Ewin.confirm({
					...
					$.ajax({
						type : "post",
						url : "category_deleteByIds.action",
						data : {
							"ids" : ids
						},
						success : function(data, status) {
