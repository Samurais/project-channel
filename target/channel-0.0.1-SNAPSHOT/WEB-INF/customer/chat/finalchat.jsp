...
<script src="<%=realpath%>/resources/jquery-ui/jquery-ui.min.js"></script>
</head>
<style>
.my-btn {
	padding: 10px 5px;
	...
	<div id="thispage">
		<div class="col-md-3">
								...
								<div class="form-group">
									...
									<div class="col-lg-5">
										<textarea class="form-control" rows="3"
											placeholder="Enter ..." name="demandotherrequest"
								...
								<div class="form-group">
									<div class="col-lg-4 col-lg-offset-1">
										<button id="checkrecommend" type="button"
											class="btn btn-block btn-primary">喜欢</button>
									</div>
									<div class="col-lg-4 col-lg-offset-2">
										<button id="checkrecommend2" type="button"
											class="btn btn-block btn-primary">不喜欢</button>
									</div>
								</div>
							</form>
						</div>
							...
							<form id="defaultForm" method="post" class="form-horizontal"
								...
								<div class="form-group">
									<div class="col-lg-4 col-lg-offset-4">
										<button id="getproductclose" type="button"
			...
			<div class="box box box-primary direct-chat direct-chat-primary"
				style="width:290px" id="chatbox">
				<!-- <div
			class="box box box-primary direct-chat direct-chat-primary direct-chat-contacts-open" style="position: absolute;z-index:90"> -->
				...
				<div class="box-body" style="height:400px">
					...
					<div id="chat_window" class="direct-chat-messages"
						style="height:400px">
						<!-- **************************这里插入会话消息********************************* -->
						<div class="attachment-block clearfix">
							<img class="attachment-img"
								src="http://2e.zol-img.com.cn/product/140_800x600/694/ceUHoNnJMCDzg.jpg"
							...
							<div class="attachment-pushed">
								...
								<div class="attachment-text">
									戴尔PowerEdge R730 机架式服务器 R730采用英特尔?至强?处理器E5-2600 v3产品系列 <a
										href="#">了解更多</a>
								</div>
								<!-- /.attachment-text -->
							</div>
						...
						<div class="attachment-block clearfix">
							...
							<div class="attachment-text">
								<br> <a id="test" href="#">如何购买最新产品?</a><br> <a
									href="#">如何获得最新产品信息?</a><br> <a href="#">如何进行物流查询?</a><br>
							</div>
							<!-- /.attachment-text -->
							<!-- /.attachment-pushed -->
						</div>
					...
					<form class="direct-chat-contacts" title="constacts_register"
						style="height:400px" hidden="hidden">
						<ul class="contacts-list">
							<li><a> <label for="inp_registerid" class="form_control">账号</label>
									<input class="form-control" id="inp_registerid"
									placeholder="账号" type="text" name="inp_registerid">
					...
					<!-- ===========================留言模块=================================== -->
					<div class="direct-chat-contacts" title="constacts_leavingmsg"
						style="height:400px" hidden="hidden">
						<ul class="contacts-list">
							<li><a> <label for="leave_title" class="control-label">留言标题</label>
									<input class="form-control" id="leave_title" placeholder="留言标题"
									type="text" name="title">

							</a></li>
						</ul>
						<ul class="contacts-list">
							<li><a> <label for="leave_content" class="control-label">留言内容</label>
									<textarea class="form-control" id="leave_content" rows="5"
										placeholder="留言内容" name="content"></textarea>
							</a></li>
						</ul>
						<ul class="contacts-list">
							<li><a> <label for="leave_serviceid"
									class="control-label">留言的客服ID</label> <input
									class="form-control" id="leave_serviceid" placeholder="没有指定客服可不填"
									type="text" name="serviceid">
							</a></li>
						</ul>
						<ul class="contacts-list">
							<li><a>
									<button id="leavemsgbtn_leave" type="button"
										class="btn btn-info pull-right">留言</button>
									<button id="leavemsgbtn_cancel" type="button"
										class="btn btn-default">取消</button>
							</a></li>
						</ul>
					</div>
				...
				<!-- /.box-body -->
				<div>
					<a class="btn btn-app my-btn" name="request"> <i
						class="fa fa-commenting"></i> 请求人工
					</a><a class="btn btn-app my-btn" name="like"> <i
						class="fa fa-heart-o"></i> 猜你喜欢
					</a><a class="btn btn-app my-btn" name="search"> <i
						class="fa fa-map-pin"></i> 问题搜索
					</a><a class="btn btn-app my-btn" name="leave"> <i
						class="fa fa-edit (alias)"></i> 问题留言
					</a>
				</div>
				<div class="box-footer">
					<div class="input-group">
						<input id="chatinput" type="text" name="message"
							placeholder="正在输入 ..." class="form-control"> <span
...
<script>
	...
	$.post('getSessionParem.action', {
		...
		if (result.productid != "") {
			$.post('product_getProduct.action', {
				...
				var div = $("#chat_window");
				div.append($msg(false, "机器人", new Date(), "<%=path%>dist/img/user1-128x128.jpg", "您对下面商品有什么疑问呢?"));
				div.append($getproduct(result));
				$("#getproductclose").unbind();
				$("#getproductclose").on('click', function() {
					$('#myModal3').modal('hide');
				});
	...
	//留言按钮
	$("#leavemsgbtn_leave").on('click', function() {
		var div = $("#chat_window");
		div.append("<p class='text-light-blue'>留言成功!</p>");
		div.scrollTop(div.scrollTop() + 1000);
		$("[title=constacts_leavingmsg]:first").hide();
		$("[title='Contacts']:first").trigger("click");
	});
	//留言取消按钮
	$("#leavemsgbtn_cancel").on('click', function() {
		$("[title=constacts_leavingmsg]:first").hide();
		$("[title='Contacts']:first").trigger("click");
	});
	//****************订单确认信息******************
	...
	function $getproduct(result) {
		...
		var recommend = '<div class="attachment-block clearfix">' +
			...
			'商品号:' + productJSON.id + '<br>' +
			'商品价格:' + productJSON.price + '<br>' +
			'<button type="button" id="getrecommend" onclick="getproduct(' + JSON.stringify(result).replace(/"/g, '&quot;') + ')" class="btn btn-block btn-primary">查看详情</button>' +
			'</div>' +
			'</div>' +
			'</div>'
		return recommend;
	...
	function websocktInit() {
		...
		webSocket = new WebSocket('ws:localhost:80/channel/websocket/' + customerid);
	...
	function button_toRobot() {
		...
		if (getCookie("customerid") == "") {
			if (date.indexOf("请求人工") != -1) {
				//弹出登录窗口
				div.append($msg(false, "机器人", new Date(), "<%=path%>dist/img/user1-128x128.jpg", "请您先登陆哦"));
				div.scrollTop(div.scrollTop() + 1000);
				$("[title='Contacts']:first").trigger("click");
				return;
			} else if (date.indexOf("猜你喜欢") != -1) {
				$.post("collaborativeFiltering_getRandomProduct.action", {
					companyid : getCookie("companyid"),
					content : date,
					msgtype_id : 2310
				}, function(result) {
				...
				return;
			} else if (date.indexOf("问题搜索") != -1) {
				//提示如何查询问题
				div.append($msg(false, "机器人", new Date(), "<%=path%>dist/img/user1-128x128.jpg", "请输入'@+问题'的格式进行问题搜索"));
				div.scrollTop(div.scrollTop() + 1000);
				return;
			} else if (date.indexOf("问题留言") != -1) {
				div.append($msg(false, "机器人", new Date(), "<%=path%>dist/img/user1-128x128.jpg", "请您留言"));
				div.scrollTop(div.scrollTop() + 1000);
				$("[title=constacts_leavingmsg]:first").show();
				$("[title='Contacts']:first").trigger("click");
			} else {
	...
	//=============================================================================
	$(function() {
		$(".my-btn[name=request]").on('click', function() {
			var div = $("#chatinput");
			div.val("请求人工");
			$("#chatbut").click();
		});
		$(".my-btn[name=like]").on('click', function() {
			var div = $("#chatinput");
			div.val("猜你喜欢");
			$("#chatbut").click();
		});
		$(".my-btn[name=search]").on('click', function() {
			var div = $("#chatinput");
			div.val("问题搜索");
			$("#chatbut").click();
		});
		$(".my-btn[name=leave]").on('click', function() {
			var div = $("#chatinput");
			div.val("问题留言");
			$("#chatbut").click();
		});
	...
	function callback(result) {
		...
		case 1210:
			$.post('product_getProduct.action', {
				...
				$recommend(result);
				$("#checkrecommend2").unbind();
				$("#checkrecommend2").on('click', function() {
					$('#myModal2').modal('hide');
				});
				$("#checkrecommend").unbind();
		...
		case 2310:
			$.post('product_getProduct.action', {
				...
				console.log("2310");
				div.append($msg(false, "机器人", new Date(), "<%=path%>dist/img/user1-128x128.jpg", "您喜不喜欢这个呢"));
				div.append($recommend(result));
				$recommend(result);
				$("#checkrecommend").on('click', function() {
					$('#myModal2').modal('hide');
		...
		case 3205:
			console.log("3205");
			webSocket = websocktInit();
			div.append("<p class='text-light-blue'>您的前面还有0个人,请稍微等待，客服马上就来~</p>");
			div.scrollTop(div.scrollTop() + 1000);
			break;
		//服务器通知客户排队队列已满
		case 3206:
...
<script>
	...
	$("#test").on('click', function() {
		console.log("触发");
		var ap = '<div class="attachment-block clearfix">' +
			'<h4 class="attachment-heading">' +
			'<a href="#">问题解答:</a>' +
			'</h4>' +
			'<div class="attachment-text">' +
			'<br>只需联系人工客服即可在线下单，商品送货上门 ' +
			'</div>' +
			'</div>'
		var div = $("#chat_window");
		div.append(ap);
