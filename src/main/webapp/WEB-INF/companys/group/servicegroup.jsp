...
<div id="thispage" title="客服分组管理<small>manager</small>">
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'http://localhost/channel/data7.json', //请求后台的URL（*）
				contentType : "application/x-www-form-urlencoded; charset=UTF-8",
				method : 'post', //请求方式（*）
				toolbar : '#toolbar', //工具按钮用哪个容器
				striped : true, //是否显示行间隔色
				...
				columns : [ {
					...
					formatter : function(value, row, index) {
						return '<button type="button" class="pull-right btn btn-primary glyphicon glyphicon-pencil" onclick="editRtight(\'' + row.id + '\')">修改组成员</button>';
					}
				} ],
				onClickRow : function(row, $element) {
					curRow = row;
