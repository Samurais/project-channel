	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'customerservice_pageQueryJoinRole.action', //请求后台的URL（*）
				...
				columns : [ {
					...
					editable : {
						...
						disabled : true,
						validate : function(v) {
							if (isNaN(v)) return '电话号码必须是数字';
							var phone = parseInt(v);
							if (phone <= 0) return '电话号码必须是整数';
						}
					}
				}, {
					field : 'csSex',
					...
					field : 'customerservicerole',
					title : '客服角色',
					formatter : function(value, row, index) {
						if (row.customerservicerole!= null) {
							console.log("这个是类型：" + value.type);
							return value.type;
						} else {
							return null;
						}
					...
					editable : {
						...
						source : function() {
							var result = [];
							$.ajax({
								url : 'customerservicerole_getTotalRole.action',
								async : false,
								type : "get",
								data : {
				...
				onEditableSave : function(field, row, oldValue, $el) {
					...
					$.ajax({
						...
						data : {
							...
							'roletype' : row.customerservicerole,
							'companyid' : getCookie("companyid"),
						},
						success : function(data, status) {
							if (status == "success") {
