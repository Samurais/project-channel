...
<div id="thispage" title="权限管理<small>manager</small>">
				...
				<div class="modal-body">
					...
					<form id="defaultForm" method="post" class="form-horizontal"
						...
						<div class="form-group">
							<label class="col-lg-3 control-label">权限编号</label>
							<div class="col-lg-5">
								<input type="text" class="form-control" name="id" />
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-3 control-label">权限类别名称</label>
							<div class="col-lg-5">
						...
						<div class="form-group">
							<div class="col-lg-4 col-lg-offset-1">
								<button id="btn_sub" type="button"
									class="btn btn-block btn-primary">添加权限</button>
							</div>
...
<script>
	...
	function ajaxFileUpload() {
		$.ajaxFileUpload({
			url : 'customerserviceright_pageQuery.action', //用于文件上传的服务器端请求地址  
			secureuri : false, //一般设置为false  
			fileElementId : 'fileupload', //文件上传空间的id属性  <input type="file" id="file" name="file" />  
			dataType : 'JSON', //返回值类型 一般设置为json  
			success : function(data, status) {
				var div = $("#defaultForm");
				$.post('customerserviceright_save.action', {
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'customerserviceright_pageQuery.action', //请求后台的URL（*）
				...
				onEditableSave : function(field, row, oldValue, $el) {
					...
					$.ajax({
						type : "post",
						url : "customerserviceright_editUpdate.action",
						data : {
							'id' : row.id,
							'type' : row.type,
						},
		...
		oInit.Init = function() {
			...
			$("#btn_delete").click(function() {
				...
				Ewin.confirm({
					...
					$.ajax({
						type : "post",
						url : "customerserviceright_deleteByIds.action",
						data : {
							"ids" : ids
						},
						success : function(data, status) {
				...
				fields : {
					id : {
						...
						validators : {
							...
							remote : {
								url : 'customerserviceright_idValid.action',
								message : '权限ID已经存在'
							},
						}
					},
