...
@Controller("webSocketPacking")
public class WebSocketPackingImpl implements WebSocketPacking {
	// ********************************************************************************
	// ==========================service对象============================================
	@Resource
	...
	public void msgFilter(Msg msg) {
		...
		default:
			 ...
			 * 过滤之后 第五步：传递消息
			 */
			InfHandle infHandle = (InfHandle) SpringUtils.getBean("infHandle");
	...
	public void sendMsg(Msg msg, Terminal terminal) {
		...
		if (terminal != null) {
			...
			WebSocketConnect webSocketConnect = (WebSocketConnect) SpringUtils.getBean("webSocketConnect");
			webSocketConnect.send(msg, statusid);
		}
	}
